...
<task type="auto">
  <name>Task 5: Create comprehensive tests including receipt generation</name>
  <files>
    ...
    apps/api/apps/receipts/tests/__init__.py
    apps/api/apps/receipts/tests/test_receipt_generation.py
  </files>
  <action>
    ...
    6. Create apps/api/apps/receipts/tests/test_receipt_generation.py:
       - Test receipt PDF endpoint returns 200 with application/pdf content-type
       - Test receipt PDF contains order number, items, and total
       - Test receipt download after order creation (POS-10 verification):
         a. Create order via API
         b. Call GET /api/v1/receipts/{order_id}/pdf/
         c. Verify response is valid PDF (check %PDF header)
         d. Verify Content-Disposition header contains receipt filename
       - Test receipt requires authentication
       - Test receipt respects multi-tenant isolation (cannot download other restaurant's receipt)

    Run: pytest apps/api/apps/orders/tests/ apps/api/apps/receipts/ apps/api/apps/qr/ -v
  </action>
  <verify>pytest apps/api/apps/orders/ apps/api/apps/receipts/ -v --tb=short (all tests pass, including receipt generation tests)</verify>
  <done>All tests pass, comprehensive coverage for orders, receipts (POS-10), QR</done>
</task>
...
6. Receipt PDF downloads correctly (GET /api/v1/receipts/{order_id}/pdf/ returns valid PDF)
...
- Receipt PDF can be downloaded for any order (POS-10 requirement)
