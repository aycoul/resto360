...
must_haves:
  truths:
    ...
    - "Completed orders are removed from display after confirmation"
  artifacts:
...
<task type="auto">
  <name>Task 3: Build order queue components with completed order removal</name>
  <files>
    apps/web/components/kitchen/OrderQueue.tsx
    apps/web/components/kitchen/OrderStatusColumn.tsx
    apps/web/components/kitchen/OrderCard.tsx
  ...
  <done>Order queue with status columns and cards ready, completed orders removed from display</done>
</task>

<task type="auto">
  <name>Task 4: Add auto-refresh, sound notifications, and completed order fade-out</name>
  <files>
    apps/web/components/kitchen/OrderQueue.tsx
    apps/web/lib/hooks/useKitchenQueue.ts
  ...
  <action>
    1. Update apps/web/lib/hooks/useKitchenQueue.ts to include sound notification and completed order removal:
       ```typescript
       'use client';
  ...
  <verify>New orders trigger sound notification, time updates every second, completed orders fade out after 2 seconds</verify>
  <done>Sound notifications, auto-refresh, and completed order removal working</done>
</task>
...
7. Completed orders fade out after 2 seconds and are removed from display
...
- Completed orders are removed from display after confirmation (2 second delay)
</success_criteria>
