...
def handle_orange_webhook(webhook_data: dict) -> Optional[Payment]:
    ...
    Note: Orange webhooks can be unreliable; polling is the primary fallback.
    ...
    if not payment_reference:
        logger.warning("Orange webhook missing payment_reference")
        return None

    logger.info(
        "Processing Orange webhook: reference=%s, status=%s",
        payment_reference,
        status,
    )
...
def handle_mtn_webhook(webhook_data: dict) -> Optional[Payment]:
    ...
    Note: MTN sandbox doesn't support callbacks; polling is essential there.
    Production may receive callbacks but polling should still be enabled.

    Args:
        webhook_data: Normalized webhook data from MTNProvider.parse_webhook()
            - event_type: Type of event
            - payment_reference: The reference ID (X-Reference-Id)
            - status: Normalized status (success, failed, pending)
            - external_id: Our order reference
    ...
    if not payment_reference:
        logger.warning("MTN webhook missing payment_reference")
        return None

    logger.info(
        "Processing MTN webhook: reference=%s, status=%s",
        payment_reference,
        status,
    )
    ...
    with transaction.atomic():
        ...
        if status == "success":
            ...
            payment.mark_failed(
                error_code=error_data.get("error_code", "unknown"),
                error_message=error_data.get("reason", "Payment failed"),
            )
            payment.provider_response = error_data
            payment.save()
            logger.info("Payment %s marked as FAILED", payment.idempotency_key)
            ...
            # MTN doesn't have expired status, just pending
            logger.info(
                "MTN webhook status %s for payment %s (no action)",
                status,
                payment.idempotency_key,
            )
            return None
